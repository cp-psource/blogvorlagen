...
class NBT_Templates_Table extends WP_List_Table {
    ...
    function column_name( $item ) {
        ...
        $actions = array(
            'edit'      => sprintf( __( '<a href="%s">Bearbeiten</a>', 'blog_templates' ), $url ),
            'delete'    => sprintf( __( '<a href="%s">Löschen</a>', 'blog_templates' ), $url_delete ),
        );

        if ( $item['is_default'] ) {
            $actions['remove_default'] = sprintf( __( '<a href="%s">Standard entfernen</a>', 'blog_templates' ), $url_remove_default );
            $default = ' <strong>' . __( '(Standard)', 'blog_templates' ) . '</strong>';
        }
        else {
            $actions['make_default'] = sprintf( __( '<a href="%s">Als Standard markieren</a>', 'blog_templates' ), $url_default );
            $default = '';
        }
    ...
    function column_blog( $item ) {
        ...
        return $name . ' <a href="' . $url . '">' . __( 'Gehe zum Dashboard', 'blog_templates' ) . '</a>';
    }
    ...
    function get_columns(){
        $columns = array(
            'name'          => __( 'Vorlagenname', 'blog_templates' ),
