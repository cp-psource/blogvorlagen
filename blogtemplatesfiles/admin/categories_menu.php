...
class blog_templates_categories_menu {
    ...
    function network_admin_page() {
        $this->page_id = add_submenu_page( 'blog_templates_main', __( 'Vorlagenkategorien', 'blog_templates' ), __( 'Vorlagenkategorien', 'blog_templates' ), 'manage_network', $this->menu_slug, array($this,'render_page'));
    }
    ...
    public function render_page() {
    	...
    	elseif ( isset( $_GET['updated'] ) ) {
    		?>
				<div class="updated">
					<p><?php _e( 'Änderungen wurden übernommen', 'blog_templates' ); ?></p>
				</div>
    		<?php
    	}
    	?>
			<div class="wrap">
				...
				<?php if ( isset( $_GET['action'] ) && 'edit' == $_GET['action'] && isset( $_GET['category'] ) && $cat_id = absint( $_GET['category'] ) ): ?>
					...
					<?php
						...
						if ( ! $category )
							wp_die( __( 'Die Kategorie existiert nicht', 'blog_templates' ) );
					...
					<form id="categories-table-form" action="" method="post">
						<table class="form-table">
							...
							<?php
								$this->render_row( __( 'Kategoriename', 'blog_templates' ), ob_get_clean() );
							?>
							...
							<?php
								$this->render_row( __( 'Kategoriebeschreibung', 'blog_templates' ), ob_get_clean() );
							?>
						</table>
				...
				<?php else: ?>
					...
					<div id="col-container">
						...
						<div id="col-left">
							<div class="col-wrap">
								<div class="form-wrap">
									<h3><?php _e( 'Neue Kategorie hinzufügen', 'blog_templates' ); ?></h3>
									<form id="categories-table-form" action="" method="post">
										<?php wp_nonce_field( 'add-nbt-category' ); ?>
										<div class="form-field">
											<label for="cat_name"><?php _e( 'Kategoriename', 'blog_templates' ); ?>
												<input name="cat_name" id="cat_name" type="text" value="<?php echo $this->current_category['name']; ?>" size="40" aria-required="true">
											</label>
										</div>
										<div class="form-field">
											<label for="cat_description"><?php _e( 'Kategoriebeschreibung', 'blog_templates' ); ?>
												<textarea name="cat_description" rows="5" cols="40"><?php echo esc_textarea( $this->current_category['description'] ); ?></textarea>
											</label>
										</div>
										<?php submit_button( __( 'Neue Kategorie hinzufügen', 'blog_templates' ), 'primary', 'submit-nbt-new-category' ); ?>
									</form>
								</div>
							</div>
						</div>
	...
	public function validate_form() {
		if ( isset( $_POST['submit-edit-nbt-category'] ) ) {
			if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'edit-nbt-category' ) )
				wp_die( __( 'Sicherheitsüberprüfungsfehler', 'blog_templates' ) );
			...
			if ( isset( $_POST['cat_name'] ) && ! empty( $_POST['cat_name'] ) && isset( $_POST['cat_id'] ) ) {
				...
				$this->errors = __( 'Der Name darf nicht leer sein', 'blog_templates' );
			}
		}
